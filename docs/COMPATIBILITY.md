...
| Packed dist/index.html | verified, 2026-09-26 | The example now uses `window.HydraEffects.loadHydraEffects()`. Served from the packed tarball in headless Chromium 154: canvas present, changing output (three distinct 64×48 samples), zero page errors. [Evidence](../workflow-evidence/gap-004/packed-html-verification.json). The 2026-09-24 failure record (`Hydra is not defined`) applied to the removed-constructor version. |
...
| 2026-09-26, distribution qualification (GAP-003) | Containing commit | `npm run build` reproduced all four bundle hashes byte-for-byte; `node --test test/*.test.mjs` 46 pass, 0 fail, 0 skipped; 57/58 sweep with the unchanged `rotate_animated_parameter` host residual (96/16,384 bytes, max delta 1); `npm pack` 32 files with a zero-difference extract-and-compare against the checkout; isolated install exit 0 and the installed ESM entry exposes the six documented exports under Node 26.5.1; isolated `npm ci` + `node --test test/installed-workflow.test.mjs` exit 0. | SPDX metadata fixed to `AGPL-3.0-only`; changelog 2.0.0-dev.0 migration entry; dependency contract and minimum runtime declared in the README; served engine identity receipt re-fetched and unchanged (`31b76609…29fbded`, Last-Modified Sat, 26 Sep 2026 01:49:01 GMT, correlated with upstream `v1.0.185`). GAP-003 remains open. [Evidence](../workflow-evidence/gap-003/distribution-qualification.json). |
